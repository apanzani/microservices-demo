...
public class TwitterQueryUserJwtConverter implements Converter<Jwt, AbstractAuthenticationToken> {
    ...
    public AbstractAuthenticationToken convert(Jwt source) {
        Collection<GrantedAuthority> authoritiesFromJwt = getAuthoritiesFromJwt(source);
        return Optional.ofNullable(
                twitterQueryUserDetailsService.loadUserByUsername(source.getClaimAsString(USERNAME_CLAIM)))
                .map(userDetails -> {
